...
namespace _6524
{
    public partial class Form1 : Form
    {
            ...
            while (!Bg_Main.CancellationPending)
            {
                ...
                PLCINIT:
                    ...
                    if (autochangemodel)
                    {
                        ...
                        //自动切换机种，机种名必须为1.2.3
                        OperateResult<int[]> model = MC_PLC.ReadInt32("D470", 1);
                        if (model.IsSuccess)
                        {
                        ...
                        if (model.Content[0].ToString() == "1")
                        {
                           // label1.BeginInvoke(new Action(() => { label1.Text = "MachineNo.211"; }));
                        }
                        if (model.Content[0].ToString() == "2")
                        {
                           // label1.BeginInvoke(new Action(() => { label1.Text = "MachineNo.108"; }));
                        }
                        if (model.Content[0].ToString() == "3")
                        {
                            //label1.BeginInvoke(new Action(() => { label1.Text = "MachineNo.210"; }));
                        }
                                        ...
                                        if (PLCinit.Content[0])
                                        {
                                            ...
                                            while (!plcinitok && !Bg_Main.CancellationPending)
                                            {
                                                ...
                                                if (PLCinit.IsSuccess)
                                                {
                                                    if (PLCinit.Content[0])
                                                    {
                                                        MC_PLC.Write(PLCinit_Path, false);
                                                        goto PLCINIT;
                                                        //跳转至读取机种
                                                    }
                                                }
        ...
        private void run(object threadName)
        {
            ...
            for (int AA = 0; AA < d1.Rows.Count; AA++)
            {
                // 注意：C# 中列的索引是从 0 开始的，所以第 7 列的索引是 6
                // 这里加上类型转换和空值判断，防止报错
                if (d1.Rows[AA][7] != DBNull.Value)
                {
                    int val = Convert.ToInt32(d1.Rows[AA][7]);
            ...
            if (count1 == imageresult1.Count && count1 != 0)
            {
                m_Logprint(HslMessageDegree.INFO, "工位", 1, "发送结果中", true);
                OperateResult Writeresult1;
                if (!imageresult1.Contains(false))
                {
                    Writeresult1 = MC_PLC.Write(d2.Rows[0][1].ToString(), new bool[] { true });  // 1工位写入OK 结果
                    m_Logprint(HslMessageDegree.INFO, "工位", 1, "发送结果OK", true);
                }
                else
                {
                    Writeresult1 = MC_PLC.Write(d2.Rows[0][2].ToString(), new bool[] { true });  // 1工位写入NG结果
            ...
            if (count2 == imageresult2.Count && count2 != 0)
            {
                m_Logprint(HslMessageDegree.INFO, "工位", 2, "发送结果中", true);
                OperateResult Writeresult2;
                if (!imageresult2.Contains(false))
                {
                    Writeresult2 = MC_PLC.Write(d2.Rows[1][1].ToString(), new bool[] { true });  // 2工位写入OK 结果
                    m_Logprint(HslMessageDegree.INFO, "工位", 2, "发送结果OK", true);
                }
                else
                {
                    Writeresult2 = MC_PLC.Write(d2.Rows[1][2].ToString(), new bool[] { true });  // 2工位写入NG结果
            ...
            if (count3 == imageresult3.Count && count3 != 0)
            {
                m_Logprint(HslMessageDegree.INFO, "工位", 3, "发送结果中", true);
                OperateResult Writeresult3;
                if (!imageresult3.Contains(false))
                {
                    Writeresult3 = MC_PLC.Write(d2.Rows[2][1].ToString(), new bool[] { true });  // 3工位写入OK 结果
                }
                else
                {
                    Writeresult3 = MC_PLC.Write(d2.Rows[2][2].ToString(), new bool[] { false });  // 3工位写入NG结果
                }
            ...
            if (count4 == imageresult4.Count && count4 != 0)
            {
                m_Logprint(HslMessageDegree.INFO, "工位", 4, "发送结果中", true);
                OperateResult Writeresult4;
                if (!imageresult4.Contains(false))
                {
                    Writeresult4 = MC_PLC.Write(d2.Rows[3][1].ToString(), new bool[] { true });  // 4工位写入OK 结果
                }
                else
                {
                    Writeresult4 = MC_PLC.Write(d2.Rows[3][2].ToString(), new bool[] { false });  // 4工位写入NG结果
                }
                    ...
                    try
                    {
                        ...
                        if (M_Shape_matching.Row.TupleLength() == M_Shape_matching.Hv_Matching_num)
                        {
                            ...
                            if (Region_enabled)
                            {
                                ...
                                disp_message(HWindowshandle, A, "image", rec_row - (rec_length1), rec_col - (rec_length2), "black", "true");
                            }
                        ...
                        else
                        {
                            ...
                            {
                                HOperatorSet.DispObj(rec_xld, HWindowshandle);
                                disp_message(HWindowshandle, A, "image", rec_row, rec_col, "black", "true");
                            }
                            return false;
                        }
...
HTuple hv_Row, HTuple hv_Column, HTuple hv_Color, HTuple hv_Box)
   ...
   out HTuple hv_Column, out HTuple hv_Radius)
            ...
            if (PLC_connected)
            {
                ...
                while (!Bg_PLC_heartbeat.CancellationPending)
                {
                    try
                    {
                        ...
                        OperateResult result2 = MC_PLC.Write(heartbeat_path, new bool[] { false });//复位拍照限信号
                        Thread.Sleep(sleeptime);
                        if (!result.IsSuccess)
                        {
        ...
        {
            HOperatorSet.ReadImage(out Img, @"C:\Users\FRANK\Downloads\11_40_05_7406.bmp");
            _mWindow1.NowImage = Img;
            runhandleimage(_mWindow1.hWindowControl.HalconWindow, Img, 201);
        }
    }
